...
int sentiment_1d_coloring(int num_nodes, const vector<int> & row_ptr, const vector<int> & col_ind, vector<pair<int, float>> & spare_order) {
	...
	vector<pair<int, int>> node_values(num_nodes, {1, 0});
	...
	do {
		...
		if(next_node.second == node.second) {
		...
		int color = 0;
		for (; color < num_nodes; color++) { // greedily choose the smallest possible color
		...
		for(int i = 0; i < num_nodes; i++) {
			node_values[i].first = i;
			if(node_colored[i]) {
				node_values[i].second = -1;
			}
			else {
				node_values[i].second = color_infos[i].size();
			}
		}

		sort(node_values.begin(), node_values.end(), descending);
...
void normal_params(vector <pair<int, float> > & order, float & mean, float & stdev){
	...
	if(isnan(stdev) || stdev == 0) stdev = 0.001;
}
