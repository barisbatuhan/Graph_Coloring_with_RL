...
int main(int argc, char **argv)
{
	...
	if (pDIR = opendir(path))
	{
		while (entry = readdir(pDIR))
		{
			...
			if (fname.at(fname.length() - 1) == '.' || g == "rbsb480.mtx" || g == "rbsa480.mtx")
			{
				continue;
			}
			gnames.push_back(g);
						...
						for (int i6 = 0; i6 < 21; i6++)
						{
							...
							{ // sum of coeff must be one
								...
								for (auto &graph : graphs)
								{
									...
									float k = graph_1d_coloring(row_ptr, col_ind, new_order);
								...
								{ // if it is the best performance keep the best coefficients
									...
									//printf("%f, %f, %f, %f, %f, %f, Total colors : %f\n", c1, c2, c3, c4, c5, c6, total_colors);
